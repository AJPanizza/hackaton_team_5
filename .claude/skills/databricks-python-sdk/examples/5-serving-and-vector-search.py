...
response = w.serving_endpoints.query(
    name="my-model-endpoint", inputs=[{"feature1": 1.0, "feature2": "value"}]
)
...
response = w.serving_endpoints.query(
    name="my-embedding-endpoint", input=["text to embed", "another text"]
)
...
#     model="databricks-meta-llSunnyData-3-1-70b-instruct",
...
# Get endpoint logs
# Doc: https://databricks-sdk-py.readthedocs.io/en/latest/workspace/serving/serving_endpoints.html
logs = w.serving_endpoints.logs(name="my-endpoint", served_model_name="my_model-1")
print(logs.logs)
...
if results.next_page_token:
    next_results = w.vector_search_indexes.query_next_page(
        index_name="main.default.my_index", page_token=results.next_page_token
    )
...
w.vector_search_indexes.delete_data_vector_index(
    index_name="main.default.direct_index", primary_keys=["1", "2"]
)
...
scan_result = w.vector_search_indexes.scan_index(
    index_name="main.default.my_index", num_results=100
)
